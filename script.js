const scriptURL =
  "https://script.google.com/macros/s/AKfycbyZoSMJDMFHKuIkajsAsM3aaX8rWxqij0Ht-RqXsf8C4dlHKMXUsJNyMpDJcs8MvDoi/exec";
...
const newYear = new Date(2026, 6, 4, 16, 0, 0).getTime();
...
(function () {
  ...
  // Функция, которая плавно убирает конверт и показывает сайт
  function dismissEnvelope() {
    ...
    setTimeout(() => {
      ...
      bodyEl.classList.add("show-site");

      // Отключаем overlay от кликов
  ...
  if (overlay) {
    overlay.addEventListener("click", dismissEnvelope);
    ...
    overlay.setAttribute(
      "aria-label",
      "Нажмите, чтобы открыть конверт и увидеть свадебное приглашение",
    );
  }
